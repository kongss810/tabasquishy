if (!customElements.get('variant-selects')) {
    ...
    class VariantSelects extends HTMLElement {
        ...
        connectedCallback() {
            ...
            this.setDisabled();
            this.setImageSet();
            if (this.closest('.product-drawer')) {
        ...
        setupScrollGuard() {
            ...
            this.addEventListener(
                'pointerdown',
                (event) => {
                    if (event.target.closest('input, select, label, .product-form__input-option')) {
                        this._drawerScroll = this.captureDrawerScroll();
                    }
                },
                true,
            );
            ...
            this.addEventListener('focusin', () => {
                if (this._drawerScroll) {
                    this.scheduleDrawerScrollRestore(this._drawerScroll);
                }
            });
        }

        getDrawerScrollContainers() {
            const drawer = this.closest('.product-drawer');
            if (!drawer) return [];

            return [
                drawer.querySelector('.side-panel-content'),
                drawer.querySelector('.product-information'),
                drawer.querySelector('.product_left'),
                drawer.querySelector('.product-thumbnail-container'),
            ].filter(Boolean);
        }

        captureDrawerScroll() {
            return this.getDrawerScrollContainers().map((el) => ({
                el,
        ...
        restoreDrawerScroll(captured) {
            if (!captured?.length) return;

            captured.forEach(({ el, scrollTop, scrollLeft }) => {
                el.scrollTop = scrollTop;
                el.scrollLeft = scrollLeft;
            });
        }

        restoreDrawerScrollState() {
            if (this._drawerScroll) {
                this.scheduleDrawerScrollRestore(this._drawerScroll);
            }
        }

        scheduleDrawerScrollRestore(captured) {
            if (!captured?.length) return;

            const restore = () => this.restoreDrawerScroll(captured);
            restore();
        ...
        onVariantChange() {
            const inDrawer = !!this.closest('.product-drawer');
            const drawerScroll = inDrawer ? this._drawerScroll || this.captureDrawerScroll() : null;
            ...
            if (inDrawer) {
                this._drawerScroll = drawerScroll?.length ? drawerScroll : this._drawerScroll;
                this.restoreDrawerScrollState();
            }
        }
        ...
        updateMedia() {
            ...
            let productSlider = this.productWrapper?.querySelector('.product-images'),
                thumbnails = this.productWrapper?.querySelector('#Product-Thumbnails');
        ...
        setActiveMedia(mediaId, thumbId, productSlider, thumbnails) {
            let flkty = Flickity.data(productSlider),
                ...
                inDrawer = !!this.closest('.product-drawer');
            ...
            if (flkty && this.hideVariants) {
                ...
                productSlider.selectCell(mediaId, inDrawer);
            } else if (flkty) {
                if (inDrawer) {
                    flkty.options.adaptiveHeight = false;
                }
            ...
            if (!inDrawer) {
                if (typeof productSlider.resize === 'function') {
                    productSlider.resize();
                } else if (flkty && typeof flkty.resize === 'function') {
                    flkty.resize();
                }
            }
        }
        ...
        renderProductInfo() {
            ...
            fetch(`${this.dataset.url}?variant=${this.currentVariant.id}&section_id=${this.dataset.section}`)
                ...
                .then((responseText) => {
                    ...
                    if (inDrawer && drawerScroll?.length) {
                        this._drawerScroll = drawerScroll;
                        this.scheduleDrawerScrollRestore(drawerScroll);
                    }
                });
        }
                ...
                (options, variant) => {
                    ...
                    Object.keys(options).forEach((index) => {
                        ...
                        if (
                            ...
                            selected_options.find((option) => option.value === entry.value && index === option.index)
                        ) {
...
if (!customElements.get('product-slider')) {
    ...
    class ProductSlider extends HTMLElement {
        ...
        setupEvents() {
            ...
            this.flkty.on('change', (index) => {
                let previous_slide = this.flkty.cells[_this.selectedIndex] ? this.flkty.cells[_this.selectedIndex].element : null,
                    ...
                    active_thumbs = Array.from(this.thumbnails).filter(
                        (element) => element.classList.contains('is-active') && !element.classList.contains('is-media-hidden'),
                    ),
                ...
                requestAnimationFrame(() => {
                    if (this.dataset.skipThumbScroll === 'true') {
                        delete this.dataset.skipThumbScroll;
                    }

                    if (this.product_container?.closest('.product-drawer')) {
                        return;
                    }
        ...
        thumbnailClick(thumbnail) {
            ...
            const active_thumbs = Array.from(this.thumbnails).filter(
                (element) => element.classList.contains('is-active') && !element.classList.contains('is-media-hidden'),
            );
        ...
        applyMediaFilter() {
            ...
            const firstVisibleThumb = thumbnails.find(
                (thumbnail) => thumbnail.classList.contains('is-active') && !thumbnail.classList.contains('is-media-hidden'),
            );
        ...
        pauseMedia(media) {
            if (media.classList.contains('product-single__media-external-video')) {
                if (media.dataset.provider === 'youtube') {
                    media.querySelector('iframe').contentWindow.postMessage(
                        JSON.stringify({
                            event: 'command',
                            func: 'pauseVideo',
                            args: '',
        ...
        selectCell(mediaId, isInstant = false) {
            if (isInstant) {
                this.dataset.skipThumbScroll = 'true';
            }
            this.flkty.selectCell(mediaId, false, isInstant);
        }
        setupProductGallery() {

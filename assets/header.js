...
if (!customElements.get('theme-header')) {
  class ThemeHeader extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // header 高度还会因窗口尺寸变化、字体/图片加载完成而改变，
      // 仅靠 scroll 更新会导致 --header-height 取值不准，这里补上 resize/load 重算
      window.addEventListener('resize', this.setHeaderHeight.bind(this), {
        passive: true
      });
      window.addEventListener('load', this.setHeaderHeight.bind(this), {
        passive: true
      });
      if (document.fonts && document.fonts.ready) {
        document.fonts.ready.then(() => this.setHeaderHeight());
      }
